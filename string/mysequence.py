...
def pal(nom)   :
    if nom.lower()==nom.lower()[::-1]: return True
    else :return False
...
def nbr_let(txt,x=None):
    ...
    if x==None:return sorted(lettre.items())
    else:return lettre[x]
...
def casse(char_liste,casse=None):
    ...
    if casse==None:return  sorted(Lw),sorted(Up)
    elif casse==0:return sorted(Lw)
    elif casse==1:return sorted(Up)
    elif casse==-1:return [m,M]
